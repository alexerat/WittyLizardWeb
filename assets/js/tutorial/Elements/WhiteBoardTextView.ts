            ...
            let lineElems = state.textNodes.map(function (textElem : TextNode)
            {
                ...
                for(let j = 0; j < textElem.sections.length; j++)
                {
                    ...
                    for(let i = 0; i < sectionData.glyphs.length; i++)
                    {
                        ...
                        let glyphArgs = {
                            key: 'glyph' + i, d: glyph.path, stroke: 'none', fill: glyph.colour,
                    ...
                    {
                        key: 'sec' + j, transform: 'scale(' + state.size / 1000 + ',' + state.size / 1000 + ')' +
                        'translate(' + sectionData.startPos * 1000 / state.size + ', 0)'
                    }, glyphs);
