                ...
                if(!state.isMoving)
                {
                    if(mode == BoardModes.SELECT)
                    {
                        items.push(React.createElement('circle',
                        {
                            ...
                            onClick: (e: React.MouseEvent<SVGElement>) => { if(e.detail == 2) { dispatcher.doubleClick(e, ViewComponents.Interaction, null); } else { dispatcher.mouseClick(e, ViewComponents.Interaction, null); } }
                        }));
                    }
