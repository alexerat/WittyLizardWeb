...
namespace WhiteBoardText {
    ...
    interface EditData {
        deletions: Array<DeletionData>;
        insertion: InsertionData;
        styleChanges: Array<StyleChangeData>;
    }
    ...
    export class Element extends BoardElement
    {
        ...
        */
        ...
        public handleKeyPress(e: KeyboardEvent, input: string, palleteState: Pallete)
        {
            ...
            case 'Backspace':
                ...
                if(this.cursorEnd > 0)
                {
                    ...
                    let prevWordData = this.wordData;
                    ...
                    else
                    {
                        ...
                        console.log('Sorting out deletion... sortedSelect: ');
            ...
            default:
                ...
                console.log('Glyph count before: ' + this.glyphCount);
                console.log('Temp glyph count: ' + tmpGlyphLength);
                ...
                console.log('Glyph count after: ' + this.glyphCount);
                console.log('Cursor start is: ' + this.cursorStart + '. Cursor end is: ' + this.cursorEnd);
                ...
                */
                ...
                this.operationStack.splice(this.operationPos - 1, this.operationStack.length - this.operationPos);
                this.operationStack[this.operationPos++] = { undo: undoOp, redo: redoOp };
            ...
            console.log('Cursor start is now: ' + this.cursorStart);
            console.log('Cursor end is now: ' + this.cursorEnd);
            console.log('String start is: ' + this.stringStart);
            console.log('Selected positions are:');
        ...
        public handlePalleteChange(pallete: Pallete, change: BoardPalleteChange)
        {
            ...
            if(change.type == PalleteChangeType.JUSTIFIED)
            {
                ...
                */
                ...
                console.warn('THIS CODE MAY BE WRONG IN SETTING THESE NULL.');
                this.wordEnd = this.operationPos;
                this.wordStart = this.operationPos;

                this.lastFowardEdit = null;

                this.cursorUndoPositions.push(null);
                this.cursorRedoPositions.push(null);
                this.prevCursorPos = this.cursorStart;
            ...
            else
            {
                ...
                let undoPositions: PreviousCursorPositions =
                {
                    start: null, end: null, prevEnd: null, bStart: null, bPrevEnd: null
                };
                let redoPositions: PreviousCursorPositions =
                {
                    start: null, end: null, prevEnd: null, bStart: null, bPrevEnd: null
                ...
                let undoOp = () => { return this.setStyleSet(undoSet, undoPositions.start, undoPositions.end); };
                let redoOp = () => { return this.setStyleSet(styles, undoPositions.start, undoPositions.end); };
                ...
                if(this.cursorStart != this.cursorEnd)
                {
                    ...
                    */
                    ...
                    undoPositions.start = this.cursorStart;
                    undoPositions.bStart = this.cursorStart;
                    undoPositions.end = this.cursorEnd;
                    undoPositions.prevEnd = this.cursorEnd;
                    undoPositions.bPrevEnd = this.cursorEnd;

                    redoPositions.start = this.cursorStart;
                    redoPositions.bStart = this.cursorStart;
                    redoPositions.end = this.cursorEnd;
                    redoPositions.prevEnd = this.cursorEnd;
                    redoPositions.bPrevEnd = this.cursorEnd;
                ...
                {
                    this.lastFowardEdit = this.operationPos;
                }
                ...
                if(loc >= line.start && loc <= line.end)
                {
                    if(loc == line.end)
                    {
                        ...
                        else if(!line.spaceRemoved)
                        {
                            ...
                            let glyphIdx = loc - lastSec.startGlyph - 1;
                            let localPos = (lastSec.glyphs[glyphIdx].startAdvance + lastSec.glyphs[glyphIdx].xAdvance);
                    ...
                    else
                    {
                        for(let j = 0; j < line.sections.length; j++)
                        {
                            ...
                            {
                                let localPos = line.sections[j].glyphs[loc - line.sections[j].startGlyph - 1].startAdvance;
                                return (line.sections[j].startPos + localPos * this.size / 1000);
                            }
                            else if(line.sections[j].startGlyph == loc)
                            {
                                return line.sections[j].startPos;
                            }
                        }
                    }
                }
        ...
        private findCursorElems()
        {
            ...
            for(let i = 0; i < this.textNodes.length; i++)
            {
                ...
                if(this.cursorEnd >= line.start && this.cursorEnd <= line.end)
                {
                    ...
                    else if(this.cursorEnd == line.end)
                    {
                        ...
                        {
                            let sec = line.sections[line.sections.length - 1];
                            let idx = this.cursorEnd - sec.startGlyph - 1;
                            selStart = sec.startPos + ((sec.glyphs[idx].startAdvance + sec.glyphs[idx].xAdvance) * this.size / 1000);
                        }
                    }
                    else
            ...
            console.log('Cursor elements are:');
            console.log(this.cursorElems);
        }
        ...
        private getStyle(startStyleNum: number, startPos: number): TextStyle
        {
            let style: TextStyle = this.styleSet[startStyleNum];
            while(style.end <= startPos)
            {
                startStyleNum++;
                style = this.styleSet[startStyleNum];
            }
            return style;
        }
        ...
        private splitSegments(style: TextStyle, wordStart: number, localStart: number, word: string)
        {
            ...
            console.log('Splitting segments.... wordStart: ' + wordStart + ' localStart: ' + localStart + ' word: ' + word);
        ...
        {
            console.log('Splitting with styles wordStart: ' + wordStart + ' localStart: ' + localStart + ' word: ' + word);
            let segments: Array<SegmentData> = [];
            ...
            while(end > style.end)
            {
                ...
                console.log('Style index is: ' + styIndex);
                console.log(style);
                segments.push(...this.splitSegments(this.styleSet[style.seq_num], wordStart, localStart, text));
                styIndex++;
         ...
         *
         */
        private processDeletions(wordNum: number, nextWord: WordData, deletions: Array<DeletionData>) : {wordCount: number, nextWord: WordData}
        {
            let word: WordData = null;
            let wordCount: number = 0;

            if(wordNum < this.wordData.length - 1)
            {
                if(nextWord == null)
                {
                    word = {
                        startPos: this.wordData[wordNum].startPos, wordAdvance: this.wordData[wordNum].wordAdvance, segments: this.wordData[wordNum].segments,
                        wordLength: this.wordData[wordNum].wordLength
                    };

                    nextWord = {
                        startPos: this.wordData[wordNum + 1].startPos, wordAdvance: this.wordData[wordNum + 1].wordAdvance, segments: this.wordData[wordNum + 1].segments,
                        wordLength: this.wordData[wordNum + 1].wordLength
                    };

                    for(let j = 0; j < deletions.length; j++)
                    {
                        this.applyDeletion(word, deletions[j]);
                        this.applyDeletion(nextWord, deletions[j]);
                    }
                }
                else
                {
            ...
            else if(this.wordData.length > 1)
            {
                word = nextWord;
                for(let j = 0; j < deletions.length; j++)
                {
                    console.log('Applying deletion.');
                    this.applyDeletion(word, deletions[j]);
                }
            }
            else
            ...
            return {wordCount: wordCount, nextWord: nextWord};
        }
        ...
        private processStyleChanges(word: WordData, styleChanges: Array<StyleChangeData>)
        {
            ...
            for(let j = 0; j < styleChanges.length; j++)
            {
                let change = styleChanges[j];
                ...
                if(change.start < word.startPos + word.wordLength && change.end > word.startPos)
                {
                    for(let k = 0; k < word.segments.length; k++)
                    {
                        ...
                        if(change.start < word.startPos + seg.startPos + seg.segmentLength && change.end > word.startPos + seg.startPos)
                        {
                        ...
                        else if(change.start >= word.startPos + seg.startPos + seg.segmentLength)
                        {
                            break;
                        }
                    }
        ...
        private recalculateSegmentAdvance(segment: SegmentData)
        {
            // TODO: This will recalculate the segmentAdvance
            
        }

        /**
         *  Recalculates the advance (graphical display length) for a word and updates this value in the wordData object.
         *  
         *  @param {WordData} word - The reference to the wordData to recalculate the advance of.
         */
        private recalculateWordAdvance(word: WordData)
        {
            let newAdvance = 0;
            let wordSegments = [];
            let currStyle = -1;
            ...
            {
                let seg = word.segments[i];

                if(seg.style.seq_num != currStyle)
                {
                    if(i > 0)
                    {
                        if(needsRecalc)
                        {
                            let newText = this.text.substring(styleStart, styleStart + styleLength);
                            let newSegments = this.splitSegments(this.styleSet[currStyle], word.startPos, styleStart, newText);
                            wordSegments.push(...newSegments);
        
                            for(let j = 0; j < newSegments.length; j++)
                            {
                                newAdvance += newSegments[j].segmentAdvance;
                            }
                        }
                        else
                        {
                            wordSegments.push(...word.segments.slice(startSeg, i));
                        }
                    }
                    
                    styleLength = 0;
                    styleStart = seg.startPos;
                    currStyle = seg.style.seq_num;
                    startSeg = i;
                }
                else if(i > 0 && word.segments[i - 1].segmentLength < MAX_SEGEMENT_LENGTH)
                {
                    needsRecalc = true;
                }

                if(seg.segmentAdvance == -1)
                {
                    needsRecalc = true;
                }
                styleLength += seg.segmentLength;
            }
            ...
            {
                wordSegments.push(...word.segments.slice(startSeg, word.segments.length));
            }

            word.wordAdvance = newAdvance;
        }
         ...
         *
         */
        private splitWordBothEnds(newWordData: Array<WordData>, insertion: InsertionData, word: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            // Find Split segment.
            ...
            for(let k = 0; k < word.segments.length; k++)
            {
                ...
                if(insertion.start >= word.startPos + segment.startPos && insertion.start < word.startPos + segment.startPos + segment.segmentLength)
                {
                    seg = segment;
                    segIndex = k;
                    break;
            ...
            // Insert word before
            let segStartText = this.text.substring(word.startPos + seg.startPos, insertion.start);
            let newStartSegments = [];
            
            if(segStartText.length > 0)
            {
                newStartSegments = this.splitSegments(this.getStyle(word.segments[0].style.seq_num, word.startPos + seg.startPos), word.startPos, seg.startPos, segStartText);
            }
            ...
            let newStartWord: WordData = {
                ...
                wordLength: insertion.start - word.startPos, segments: [...word.segments.slice(0, segIndex), ...newStartSegments]
            };

            newWordData.push(newStartWord);
            wordCount++;

            let retVal = this.checkForNewlines(0, insertion.text.length, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
            wordCount = retVal.wordCount;
            totalCount = retVal.totalCount;
            currentLine = retVal.currentLine;

            // Insert new word for split segment after.
            let newEndStart = insertion.start + insertion.text.length;
            let segEndText = this.text.substring(newEndStart, seg.segmentLength + insertion.text.length + seg.startPos + word.startPos);

            console.log('Splitting segment after space.... newEndStart: ' + newEndStart + ' newEndText: ' + segEndText);
            console.log('Segment length: ' + seg.segmentLength + ' insertStart: ' + insertion.start + ' wordStart: ' + word.startPos + ' segStart: ' + seg.startPos);
            console.log('End should be: ' + (seg.segmentLength + insertion.text.length + seg.startPos + word.startPos));

            let newEndSegments = this.splitSegments(this.getStyle(word.segments[0].style.seq_num, newEndStart), newEndStart, 0, segEndText);
            let endWordAdvance: number = 0;

            for(let k = segIndex + 1; k < word.segments.length; k++)
            {
                endWordAdvance += word.segments[k].segmentAdvance;
                word.segments[k].startPos -= (insertion.start - word.startPos);
            }
            ...
            if(segIndex < word.segments.length - 1)
            {
            ...
            return {wordCount: wordCount, totalCount: totalCount, currentLine: currentLine};
        }

        private insertAtEnd(newWordData: Array<WordData>, insertion: InsertionData, insertData: Array<InsertData>, word: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            let insertStyle: TextStyle = insertion.style;
            let insert = insertData[0];
            console.log('and in here.');

            // Insert is at the end of this word.
            if(insertion.text.charAt(0).match(/\s/))
            {
                ...
                let retVal = this.checkForNewlines(0, insert.start, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
                wordCount = retVal.wordCount;
                totalCount = retVal.totalCount;
                currentLine = retVal.currentLine;
                ...
                let newWord: WordData = {
                    startPos: insert.start + insertion.start, wordAdvance: wordAdvance,
            ...
            else
            {
                ...
                if(sty1.style == sty2.style && sty1.weight == sty2.weight)
                {
                    ...
                    let styIndex = word.segments[0].style.seq_num;
                    while(styIndex < this.styleSet.length && this.styleSet[styIndex].end <= (word.startPos + seg.startPos))
                    {
                        styIndex++;
                    }

                    // Extend the last segment of this word.
                    let newText = this.text.substring(word.startPos + seg.startPos, word.startPos + seg.startPos + seg.segmentLength + insert.word.length);
                    let newSegments = this.splitSegments(this.styleSet[styIndex], word.startPos, seg.startPos, newText);
                ...
                else
                {
                    ...
                    let newText = this.text.substring(insertion.start, insertion.start + insert.word.length);
                    let newSegments = this.splitSegments(insertStyle, word.startPos, insertion.start - word.startPos, newText);
        ...
        private insertInMiddleStart(newWordData: Array<WordData>, insertion: InsertionData, insertData: Array<InsertData>, word: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            let insertStyle: TextStyle = insertion.style;
            let insert = insertData[0];

            console.log('Went in this middle of word condition.');
            let seg: SegmentData = null;
            let segIndex = -1;
            ...
            {
                let segment = word.segments[k];
                if(insertion.start == word.startPos + segment.startPos + segment.segmentLength)
                {
                    if(segIndex == -1)
                    {
                        seg = segment;
                    ...
                    {
                        console.log("GOT THE CORRECT STYLE.");
                        seg = segment;
                        segIndex = k;
                        break;
                    }
                }
                else if(insertion.start > word.startPos + segment.startPos && insertion.start <= word.startPos + segment.startPos + segment.segmentLength)
                {
                    seg = segment;
                    segIndex = k;
                    break;
            ...
            if(insertData.length == 1)
            {
                ...
                {
                    // Check for new lines at the start of the insertion.
                    let retVal = this.checkForNewlines(0, insertion.start, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
                    wordCount = retVal.wordCount;
                    totalCount = retVal.totalCount;
                    currentLine = retVal.currentLine;

                    let segStartText = this.text.substring(word.startPos + seg.startPos, insertion.start);
                    let newStartSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, seg.startPos, segStartText);
                    let startWordAdvance: number = 0;
                    ...
                    if(insertion.text.charAt(insertion.text.length).match(/\s/))
                    {
                        ...
                        let insertWord: WordData = {
                            startPos: insertion.start, wordAdvance: insertWordAdvance,
                        ...
                        let spacesEnd = insertion.start + insert.word.length;
                        let retVal = this.checkForNewlines(spacesStart, spacesEnd, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
                        wordCount = retVal.wordCount;
                        totalCount = retVal.totalCount;
                        currentLine = retVal.currentLine;
                        ...
                        // Insert new word for split segment after.
                        let newEndStart = insertion.start + insertion.text.length;
                        let segEndText = this.text.substring(newEndStart, seg.segmentLength - (insertion.start - (seg.startPos + word.startPos)));
                        let newEndSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, 0, segEndText);
                        let endWordAdvance: number = 0;
                    ...
                    else
                    {
                        ...
                        if(sty1.style == sty2.style && sty1.weight == sty2.weight)
                        {
                            ...
                            let newLength = (seg.segmentLength - (insertion.start - (seg.startPos + word.startPos))) + insert.word.length;
                            ...
                            let newText = this.text.substring(insertion.start, insertion.start + newLength);
                            ...
                            let newEndWord: WordData = {
                                startPos: insertion.start, wordAdvance: endWordAdvance,
                        ...
                        else
                        {
                            let includeNextSeg = false;
                            let newSegments = this.splitSegments(insertStyle, word.startPos, 0, insert.word);
                            let splitStart = insertion.start + insert.word.length;
                            let splitLength = seg.segmentLength - (insertion.start - (seg.startPos + word.startPos));
                            ...
                            let splitSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, insert.word.length, splitText);
                            let endWordAdvance: number = 0;
                ...
                else if(insertion.text.charAt(insertion.text.length -1).match(/\s/))
                {
                    // Check for merger of start segments
                    if(sty1.style == sty2.style && sty1.weight == sty2.weight)
                    {
                        // Extend this segment
                        let newText = this.text.substring(segIndex, insertion.start + insert.word.length);
                        ...
                        let newStartWord: WordData = {
                            startPos: word.startPos, wordAdvance: startWordAdvance,
                            wordLength: insertion.start - word.startPos + insert.word.length,
                    ...
                    {
                        let newSegments = this.splitSegments(this.getStyle(word.segments[0].style.seq_num, word.startPos + seg.startPos), word.startPos, insertion.start - word.startPos, insert.word);
                        let splitText = this.text.substring(word.startPos + seg.startPos, insertion.start);
                        let splitSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, seg.startPos, splitText);
                        let startWordAdvance: number = 0;
                        let newLength: number = 0;
                        ...
                        let newstartWord: WordData = {
                            startPos: insertion.start, wordAdvance: startWordAdvance,
                    ...
                    let newSegEnd = word.startPos + seg.startPos + seg.segmentLength + insertion.text.length;
                    let segEndText = this.text.substring(insertion.start + insertion.text.length, newSegEnd);
                    let newEndSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, 0, segEndText);
                    let endWordAdvance: number = 0;
                    ...
                    {
                        endWordAdvance += word.segments[k].segmentAdvance;
                        word.segments[k].startPos -= insertion.start - (seg.startPos + word.startPos);
                    }
                    ...
                    let newEndWord: WordData = {
                        startPos: insertion.start + insertion.text.length, wordAdvance: endWordAdvance,
                        wordLength: newSegEnd - (insertion.start + insertion.text.length), segments: endSegments
                    };
                ...
                else
                {
                    ...
                    if(sty1.style == sty2.style && sty1.weight == sty2.weight)
                    {
                        ...
                        let newText = this.text.substring(word.startPos + seg.startPos, word.startPos + seg.startPos + splitLength);
                        let newSegments = this.splitSegments(this.getStyle(word.segments[0].style.seq_num, word.startPos + seg.startPos), word.startPos, seg.startPos, newText);
                        ...
                        for(let k = segIndex; k < word.segments.length; k++)
                        {
                            console.warn('Pushed Forward.');
                    ...
                    {
                        console.log('Splicing in the new segment.');
                        console.log(word.startPos);
                        console.log(seg.startPos);
                        console.log(insertion.start);
                        // Insert new segment splitting segment.
                        let startText = this.text.substring(word.startPos + seg.startPos, insertion.start);

                        console.log('Data being passed to new start split... wordStart: ' + word.startPos + ' localStart: ' + seg.startPos + ' text: ' + startText);
                        console.log('Style set provided was:');
                        console.log(this.styleSet[word.segments[0].style.seq_num]);
                        let newStartSegments = this.splitSegments(this.getStyle(word.segments[0].style.seq_num, word.startPos + seg.startPos), word.startPos, seg.startPos, startText);
                        console.log('New Start Segment is:');
                        console.log(newStartSegments);

                        console.log('Data being passed to new split... wordStart: ' + word.startPos + ' localStart: ' + (insertion.start - word.startPos) + ' text: ' + insert.word);
                        let newSegments = this.splitSegments(insertStyle, word.startPos, insertion.start - word.startPos, insert.word);
                        console.log('New Segment is:');
                        console.log(newSegments);

                        let numExtraSegs = 0;
                        let newEndPos = word.startPos + seg.startPos + seg.segmentLength + insert.word.length;
                        ...
                        let endLocalStart = insertion.start - word.startPos + insert.word.length;
                        let endText = this.text.substring(insertion.start + insert.word.length, newEndPos);

                        let newEndSegments = [];
                        // This is really just checking that we aren't straddling two different segments with a different style to the insert.
                        if(endText.length > 0)
                        {
                            console.log('Data being passed to new end split... wordStart: ' + word.startPos + ' localStart: ' + seg.startPos + ' text: ' + startText);
                            newEndSegments = this.splitSegments(this.styleSet[insertStyle.seq_num + 1], word.startPos, endLocalStart, endText);
                            console.log('New end Segment is:');
                            console.log(newStartSegments);
                        }
                        ...
                        let tmpIndex = segIndex + numExtraSegs;
                        if(tmpIndex + 1 < word.segments.length)
                        {
            ...
            else
            {
                if(insertion.text.charAt(0).match(/\s/))
                {
                    ...
                    let newSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, 0, insert.word);
                    let insertWordAdvance: number = 0;
                ...
                else
                {
                    ...
                    {
                        // Merge segments.
                        let newWord = this.text.substring(word.startPos + seg.startPos, insertion.start + insert.word.length);
                        ...
                        newSegments.push(...this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, seg.startPos, newWord));
                    }
                    else
                    {
                        // Add new segment.
                        let cutText = this.text.substring(word.startPos + seg.startPos, insertion.start);
                        let cutSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, seg.startPos, cutText);
                        ...
                        newSegments.push(...this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, insertion.start - word.startPos, insert.word));
                    }
                    ...
                    let insertWord: WordData = {
                        ...
                        wordLength: insertion.start + insert.word.length - word.startPos, segments: newSegments
                    };
        ...
        private insertAtStart(newWordData: Array<WordData>, insertion: InsertionData, insertData: Array<InsertData>, word: WordData, nextWord: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number, nextWord: WordData}
        {
            let insertStyle: TextStyle = insertion.style;
            let insert = insertData[insertData.length - 1];

            console.log('Did the start insert thing.');
            if(insertion.text.charAt(insertion.text.length - 1).match(/\s/))
            {
                // Word is seperate so just insert new word.
                let newSegments = this.splitSegments(insertStyle, word.startPos, 0, insert.word);
                let wordAdvance: number = 0;
                ...
                let spacesEnd = insertion.start + insertion.text.length;
                let retVal = this.checkForNewlines(spacesStart, spacesEnd, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
                wordCount = retVal.wordCount;
                totalCount = retVal.totalCount;
                currentLine = retVal.currentLine;
                ...
                word.startPos += insertion.text.length;
                newWordData.push(word);
                wordCount++;
            }
            else
            {
                ...
                if(sty1.style == sty2.style && sty1.weight == sty2.weight)
                {
                    ...
                    console.log("Insert word length was: " + insert.word.length);
                    word.wordLength += insert.word.length;
                    word.wordAdvance += wordAdvance;
                ...
                else
                {
                    ...
                    let newText = this.text.substring(insertion.start + insert.start, insertion.start + insert.start + newLength);
                    let newSegments = this.splitSegments(insertStyle, word.startPos, 0, newText);
                    ...
                    {
                        wordAdvance += newSegments[k].segmentAdvance;
                        word.wordLength += newSegments[k].segmentLength;
                    }
                    for(let k = 0; k < word.segments.length; k++)
                    {
                        word.segments[k].startPos += insert.word.length;
            ...
            return {wordCount: wordCount, totalCount: totalCount, currentLine: currentLine, nextWord: nextWord};
        }

        private insertAtMiddleEnd(newWordData: Array<WordData>, insertion: InsertionData, insertData: Array<InsertData>, word: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            let insertStyle: TextStyle = insertion.style;
            let insert = insertData[insertData.length - 1];
            ...
            if(insertion.text.charAt(insertion.text.length).match(/\s/))
            {
                ...
                let insertWord: WordData = {
                    startPos: insertion.start + insert.start, wordAdvance: insertWordAdvance,
                    wordLength: insert.word.length, segments: newSegments
                };
                ...
                let cutStart = insertion.start + insertion.text.length;
                let cutLength = seg.startPos + seg.segmentLength + word.startPos - insertion.start;
                let cutText = this.text.substring(cutStart, cutStart + cutLength);
                let cutSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, 0, cutText);
                ...
                for(let k = segIndex + 1; k < word.segments.length; k++)
                {
                    if(word.segments[k].segmentAdvance == -1)
                    {
                        let segText = this.text.substring(word.startPos + word.segments[k].startPos, word.startPos + word.segments[k].startPos + word.segments[k].segmentLength);
                        let newSegs = this.splitSegments(this.styleSet[word.segments[k].style.seq_num], word.startPos, word.segments[k].startPos, segText);
                        word.segments.splice(k, 1, ...newSegs);
                    }
                ...
                let cutWord: WordData = {
                    ...
                    wordLength: word.wordLength - (insertion.start - word.startPos), segments: cutSegments
                };
            ...
            else
            {
                ...
                {
                    // Merge segments.
                    let newWord = this.text.substring(insertion.start + insert.start, insertion.start + insert.start + insert.word.length + cutLength);
                ...
                {
                    // Add new segment.
                    let cutText = this.text.substring(insertion.start + insert.start + insert.word.length, insertion.start + insert.start + insert.word.length + cutLength);
                    let cutSegments = this.splitSegments(this.styleSet[seg.style.seq_num], word.startPos, insert.word.length, cutText);
                ...
                let insertWord: WordData = {
                    startPos: word.startPos, wordAdvance: insertWordAdvance,
                    wordLength: insert.word.length + word.wordLength - (insertion.start + insert.start - word.startPos), segments: newSegments
                };
        ...
        private checkForNewlines(spacesStart: number, spacesEnd: number, newLinePositions: Array<number>, newLineData: Array<LineData>, 
            insertion: InsertionData, wordCount: number, totalCount: number, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            let startSpaces = insertion.text.substring(spacesStart, spacesEnd);
            let newLineIndex = startSpaces.indexOf('\n');
            let runningPos = spacesStart;
            while(newLineIndex >= 0)
            {
                runningPos += newLineIndex;
                startSpaces = startSpaces.substring(newLineIndex + 1, startSpaces.length);
        ...
        private insertIsolated(newWordData: Array<WordData>, insertion: InsertionData, insertData: Array<InsertData>, word: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number, startPoint: number, endPoint: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            let insertStyle: TextStyle = insertion.style;

            if(insertData.length > 1 && startPoint > 0)
            {
                // Check for new lines.
                let spacesStart = insertData[0].start + insertData[0].word.length;
                let spacesEnd = insertData[1].start;
                let retVal = this.checkForNewlines(spacesStart, spacesEnd, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
                wordCount = retVal.wordCount;
                totalCount = retVal.totalCount;
                currentLine = retVal.currentLine;
            ...
            for(let j = 0 + startPoint; j < insertData.length - endPoint; j++)
            {
                ...
                let newWord: WordData = {
                    startPos: insertData[j].start + insertion.start, wordAdvance: wordAdvance,
                ...
                if(j < insertData.length - 1)
                {
                    ...
                    let spacesEnd = insertData[j + 1].start;
                    let retVal = this.checkForNewlines(spacesStart, spacesEnd, newLinePositions, newLineData, insertion, wordCount, totalCount, currentLine);
                    wordCount = retVal.wordCount;
                    totalCount = retVal.totalCount;
                    currentLine = retVal.currentLine;
            ...
            if(insertData.length > 1 && endPoint > 0)
            {
                // Check for new lines.
        ...
        private insertInSpaces(newWordData: Array<WordData>, insertion: InsertionData, insertData: Array<InsertData>, word: WordData, wordCount: number, totalCount: number, 
            newLinePositions: Array<number>, newLineData: Array<LineData>, currentLine: number) : {wordCount: number, totalCount: number, currentLine: number}
        {
            if(insertion.text.charAt(0).match(/\s/))
            {
            ...
            let retVal = this.insertIsolated(newWordData, insertion, insertData, word, wordCount, totalCount, newLinePositions, newLineData, currentLine, 0, 0);
            wordCount = retVal.wordCount;
            totalCount = retVal.totalCount;
            currentLine = retVal.currentLine;
        ...
        private evaluateChanges(editData: EditData)
        {
            ...
            let insertData: Array<InsertData> = [];
            ...
            let spaceHandled: boolean = false;

            for(let i = 0; i < this.wordData.length; i++)
            {
                word = this.wordData[i];
                let retVal = this.processDeletions(i, nextWord, editData.deletions);
                i += retVal.wordCount;
                nextWord = retVal.nextWord;
                ...
                if(insertData.length == 0 && editData.insertion != null && editData.insertion.text.length > 0)
                {
                    ...
                    if(word.startPos >= editData.insertion.start)
                    {
                        ...
                        {
                            spaceHandled = true;
                            let retVal = this.checkForNewlines(0, editData.insertion.text.length, newLinePositions, newLineData, editData.insertion, wordCount, totalCount, currentLine);
                            wordCount = retVal.wordCount;
                            totalCount = retVal.totalCount;
                            currentLine = retVal.currentLine;
                        }
                        
                        // Word was after space
                        word.startPos += editData.insertion.text.length;
                        newWordData.push(word);
                        wordCount++;
                        inserted = true;
                    }
                    else if(word.startPos + word.wordLength <= editData.insertion.start)
                    {
                        // Word before space
                        newWordData.push(word);
                        wordCount++;
                    ...
                    {
                        // Word sliced by space
                        let retVal = this.splitWordBothEnds(newWordData, editData.insertion, word, wordCount, totalCount, newLinePositions, newLineData, currentLine);
                ...
                else if(word.wordLength > 0 && editData.insertion != null)
                {
                    console.log('Made it here and new word data is:');
                    console.log(newWordData);
                    ...
                    if(editData.insertion.start >= word.startPos && editData.insertion.start <= word.startPos + word.wordLength)
                    {
                        ...
                        {
                            let retVal = this.insertAtEnd(newWordData, editData.insertion, insertData, word, wordCount, totalCount, newLinePositions, newLineData, currentLine);
                            wordCount = retVal.wordCount;
                            totalCount = retVal.totalCount;
                            currentLine = retVal.currentLine;
                        }
                        ...
                        {
                            let retVal = this.insertInMiddleStart(newWordData, editData.insertion, insertData, word, wordCount, totalCount, newLinePositions, newLineData, currentLine);
                            wordCount = retVal.wordCount;
                            totalCount = retVal.totalCount;
                            currentLine = retVal.currentLine;
                        }
                        ...
                        let retVal = this.insertIsolated(newWordData, editData.insertion, insertData, word, wordCount, totalCount, newLinePositions, newLineData, currentLine, 1, 1);
                        wordCount = retVal.wordCount;
                        totalCount = retVal.totalCount;
                        currentLine = retVal.currentLine;
                        ...
                        {
                            let retVal = this.insertAtStart(newWordData, editData.insertion, insertData, word, nextWord, wordCount, totalCount, newLinePositions, newLineData, currentLine);
                            wordCount = retVal.wordCount;
                            totalCount = retVal.totalCount;
                            currentLine = retVal.currentLine;
                            nextWord = retVal.nextWord;
                        ...
                        {
                            let retVal = this.insertAtMiddleEnd(newWordData, editData.insertion, insertData, word, wordCount, totalCount, newLinePositions, newLineData, currentLine);
                            wordCount = retVal.wordCount;
                            totalCount = retVal.totalCount;
                            currentLine = retVal.currentLine;
                        }
                    ...
                    else if(!inserted && editData.insertion.start < word.startPos)
                    {
                        ...
                        let retVal = this.insertInSpaces(newWordData, editData.insertion, insertData, word, wordCount, totalCount, newLinePositions, newLineData, currentLine);
                        wordCount = retVal.wordCount;
                        totalCount = retVal.totalCount;
                        currentLine = retVal.currentLine;
                ...
                {
                    // There was no insert so just push the words
                    if(word.wordAdvance == -1)
                    {
                        // TODO: This is a recalculation flag, handle it! This also needs to be fed into alllll the other conditions.
                        this.recalculateWordAdvance(word);
                    }
                ...
                {
                    this.processStyleChanges(word, editData.styleChanges);
                    newWordData.push(word);
                    wordCount++;
                }
            }
            ...
            {
                console.log('Actually did this shizzle.');

                // Push the current line foward to include this insert.
                while(newLinePositions[oldLineIndex] < editData.insertion.start)
                {
                    newLineData[currentLine] = { startWord: totalCount, count: wordCount };
                    currentLine++;
                    oldLineIndex++;
                ...
                // Do inserted spaces before inserted words.
                let end = insertData.length > 0 ? insertData[0].start : editData.insertion.text.length;
                let retVal = this.checkForNewlines(0, end, newLinePositions, newLineData, editData.insertion, wordCount, totalCount, currentLine);
                wordCount = retVal.wordCount;
                totalCount = retVal.totalCount;
                currentLine = retVal.currentLine;
                ...
                for(let j = 0; j < insertData.length; j++)
                {
                    ...
                    // Handle spaces between
                    if(j < insertData.length - 1)
                    {
                        let retVal = this.checkForNewlines(insertData[j].start, insertData[j + 1].start, newLinePositions, newLineData, editData.insertion, wordCount, totalCount, currentLine);
                        wordCount = retVal.wordCount;
                        totalCount = retVal.totalCount;
                        currentLine = retVal.currentLine;
                    }
                ...
                {
                    let retVal = this.checkForNewlines(insertData[insertData.length - 1].start, editData.insertion.text.length, newLinePositions, newLineData, editData.insertion, wordCount, totalCount, currentLine);
                    wordCount = retVal.wordCount;
                    totalCount = retVal.totalCount;
                    currentLine = retVal.currentLine;
                }
            ...
            // Throw the last line in the mix
            newLineData[currentLine] = { startWord: totalCount, count: wordCount };
        ...
        /** Generates the text nodes (lines) needed to render this text.
         *
         *
         */
        private calculateTextLines()
        {
            ...
            for(let k = 0; k < this.lineData.length; k++)
            {
                ...
                // This covers empty lines.
                tspanEl =
                {
                    x: this.x, y: currY, dx: 0, dy: dy, start: prevGlyphPos, end: 0, endStringPos: endStringPos,
                ...
                while(wordNum < wordCount)
                {
                    ...
                    else if(startPos != endPos && firstLine)
                    {
                        insertSpace = true;
                    }
                    ...
                    if(insertSpace)
                    {
                        if(prevSlicePos > 0)
                        {
                            let start = prevSlicePos;
                            let end = this.text.length;
                            if(wordNum == 0)
                            {
                                end = this.wordData[wordIdx].startPos;
                            }
                            else if(wordIdx + 1 < this.wordData.length)
                            {
                                end = this.wordData[wordIdx + 1].startPos;
                            }
                            ...
                            if(spaceAdvance > this.width * 1000 / this.size)
                            {
                                let tmpAdvance = 0;
                                // Find the point to split
                                for(let j = 0; j < glyphs.length; j++)
                                {
                                    if(tmpAdvance + glyphs[j].xAdvance > this.width * 1000 / this.size)
                                    {
                                        slicePos = j + 1;
                                ...
                                let newSec: Section = {
                                    startPos: currentAdvance - sliceAdvance, glyphs: glyphs.slice(0, slicePos - 1),
                                    startGlyph: lineGlyphCount + glyphCount, stringStart: start
                                };
                                tspanEl.sections.push(newSec);
                                lineGlyphCount += slicePos - 1;
                                endStringPos = start + slicePos - 1;
                                ...
                                currentAdvance += tmpAdvance * this.size / 1000;
                            ...
                            else
                            {
                                ...
                                // Just add spaces.
                                let newSec: Section = {
                                    startPos: currentAdvance - sliceAdvance, glyphs: glyphs,
                                    startGlyph: lineGlyphCount + glyphCount, stringStart: start
                                };
                                tspanEl.sections.push(newSec);
                                ...
                                currentAdvance += spaceAdvance * this.size / 1000;
                        ...
                        if(prevSlicePos > 0 || sliceSeg > 0)
                        {
                            if(word.wordAdvance - sliceAdvance > this.width)
                            {
                                ...
                                if(word.segments[segIndex].hasHyphen)
                                {
                                    ...
                                    for(let i = 0; i < seg.glyphs.length && tmpAdvance + glyphAdvance + seg.glyphs[i].xAdvance * this.size / 1000 <= this.width; i++)
                                    {
                                        ...
                                        glyphAdvance += seg.glyphs[i].xAdvance * this.size / 1000;
                                    }
                                }
                                ...
                                if(tmpAdvance + word.segments[segIndex].segmentAdvance >= this.width)
                                {
                                    glyphAdvance = 0;
                                    let seg = word.segments[segIndex];
                                    while(slicePos < seg.glyphs.length && tmpAdvance + glyphAdvance + seg.glyphs[slicePos].xAdvance * this.size / 1000 <= this.width)
                                    {
                                        slicePos++;
                                        sliceSeg = segIndex;
                                        glyphAdvance += seg.glyphs[slicePos].xAdvance * this.size / 1000;
                                    }
                                    endPos = slicePos;
                                }

                                let newSec: Section = {
                                    startPos: currentAdvance - sliceAdvance, glyphs: word.segments[segIndex].glyphs.slice(prevSlicePos, endPos),
                                    startGlyph: lineGlyphCount + glyphCount, stringStart: word.startPos + word.segments[segIndex].startPos + word.segments[segIndex].glyphs[prevSlicePos].stringPositions[0]
                                };
                                tspanEl.sections.push(newSec);

                                if(tmpAdvance + word.segments[segIndex].segmentAdvance >= this.width)
                                {
                                    tmpAdvance += glyphAdvance;
                                }
                                else
                                {
                                ...
                                let endGlyph = word.segments[segIndex].glyphs[endPos - 1];
                                endStringPos = endGlyph.stringPositions[endGlyph.stringPositions.length - 1];
                                ...
                                for(segIndex = sliceSeg + 1; segIndex < word.segments.length && tmpAdvance < this.width; segIndex++)
                                {
                                    ...
                                    let newSec: Section = {
                                        startPos: tmpAdvance - sliceAdvance, glyphs: word.segments[segIndex].glyphs.slice(0, endPos),
                                        startGlyph: lineGlyphCount + glyphCount, stringStart: word.startPos + word.segments[segIndex].startPos + word.segments[segIndex].glyphs[0].stringPositions[0]
                                    };
                                    tspanEl.sections.push(newSec);
                            ...
                            else
                            {
                                ...
                                let newSec: Section = {
                                    ...
                                    startGlyph: lineGlyphCount + glyphCount, stringStart: word.startPos + word.segments[sliceSeg].startPos + word.segments[sliceSeg].glyphs[slicePos].stringPositions[0]
                                };

                                tspanEl.sections.push(newSec);
                                currentAdvance += word.segments[sliceSeg].segmentAdvance;
                                let endGlyph = word.segments[sliceSeg].glyphs[secEnd - 1];
                                endStringPos = endGlyph.stringPositions[endGlyph.stringPositions.length - 1];
                                ...
                                {
                                    let newSec: Section = {
                                        startPos: currentAdvance - sliceAdvance, glyphs: word.segments[j].glyphs,
                                        startGlyph: lineGlyphCount + glyphCount, stringStart: word.startPos + word.segments[j].startPos + word.segments[j].glyphs[0].stringPositions[0]
                                    };

                                    tspanEl.sections.push(newSec);
                                    currentAdvance += word.segments[j].segmentAdvance;
                                    lineGlyphCount += word.segments[j].glyphs.length;
                                    let endGlyph = word.segments[j].glyphs[word.segments[j].glyphs.length - 1];
                                    endStringPos = endGlyph.stringPositions[endGlyph.stringPositions.length - 1];
                                }
                            ...
                            if(word.wordAdvance > this.width)
                            {
                                ...
                                for(segIndex = 0; segIndex < word.segments.length && tmpAdvance < this.width; segIndex++)
                                {
                                    ...
                                    if(word.segments[segIndex].hasHyphen)
                                    {
                                        ...
                                        for(let i = 0; i < seg.glyphs.length && tmpAdvance + glyphAdvance + glyph.xAdvance * this.size / 1000 <= this.width; i++)
                                        {
                                            glyph = seg.glyphs[i];
                                            if(seg.glyphs[i].isHyphen)
                                            {
                                            ...
                                            glyphAdvance += glyph.xAdvance * this.size / 1000;
                                        }
                                    }
                                    ...
                                    if(tmpAdvance + word.segments[segIndex].segmentAdvance >= this.width)
                                    {
                                        ...
                                        while(slicePos < seg.glyphs.length && tmpAdvance + glyphAdvance + glyph.xAdvance * this.size / 1000 <= this.width)
                                        {
                                            slicePos++;
                                            sliceSeg = segIndex;
                    ...
                    while(!lineComplete && wordNum < wordCount)
                    {
                        if(insertSpace)
                        {
                            let start = this.wordData[wordIdx].startPos + this.wordData[wordIdx].wordLength;
                            let end = this.text.length;
                            if(wordIdx + 1 < this.wordData.length)
                            {
                                end = this.wordData[wordIdx + 1].startPos;
                            }
                            let text = this.text.substring(start, end);
                            ...
                            if(currentAdvance + spaceAdvance * this.size / 1000 > this.width)
                            {
                                let tmpAdvance = 0;
                                // Find the point to split
                                for(let j = 0; j < glyphs.length; j++)
                                {
                                    if(currentAdvance + tmpAdvance + glyphs[j].xAdvance * this.size / 1000 > this.width)
                                    {
                                        slicePos = j + 1;
                                    ...
                                    {
                                        tmpAdvance += glyphs[j].xAdvance * this.size / 1000;
                                    }
                                }
                        ...
                        else
                        {
                            ...
                            if(currentAdvance + word.wordAdvance > this.width)
                            {
                                let tmpAdvance = 0;
                                let segIndex = 0;
                                // This word will need splitting.
                                ...
                                for(segIndex = 0; segIndex < word.segments.length && tmpAdvance < this.width; segIndex++)
                                {
                                    ...
                                    let newSec: Section = {
                                        startPos: tmpAdvance, glyphs: word.segments[segIndex].glyphs.slice(0, endPos),
                                        startGlyph: lineGlyphCount + tmpGlyphCount + glyphCount, stringStart: word.startPos + word.segments[segIndex].startPos + word.segments[segIndex].glyphs[0].stringPositions[0]
                                    };
                                    sections.push(newSec);
                                    tmpGlyphCount += endPos;
                                    if(tmpAdvance + word.segments[segIndex].segmentAdvance >= this.width)
                                    {
                                        tmpAdvance += glyphAdvance;
                                    }
                                ...
                                if(sliceSeg != -1)
                                {
                                    tspanEl.spaceRemoved = false;
                                    sliceAdvance = tmpAdvance;
                                    currentAdvance += sliceAdvance;
                                    lineGlyphCount += tmpGlyphCount;
                                    tspanEl.sections.push(...sections);
                                }
                                else
                                {
                                    // Check for spaces and if so remove a space to adjust.
                                    if(tspanEl.sections.length > 0)
                                    {
                                        numSpaces--;
                                        let lastSec = tspanEl.sections[tspanEl.sections.length - 1];
                                        let lastGlyph = lastSec.glyphs[lastSec.glyphs.length - 1];

                                        currentAdvance -= lastGlyph.xAdvance * this.size / 1000;

                                        if(lastSec.glyphs.length == 1)
                                        {
                                            tspanEl.sections.splice(tspanEl.sections.length - 1, 1);
                                        }
                                        else
                                        {
                                            lastSec.glyphs.splice(lastSec.glyphs.length - 1, 1);
                                        }
                    ...
                    if(tspanEl.justified && lineComplete)
                    {
                        let reqAdjustment = this.width - currentAdvance;
                        let extraSpace = reqAdjustment / numSpaces;
                        let currentDist = 0;
                        ...
                        for(let j = 0; j < tspanEl.sections.length; j++)
                        {
                            ...
                            if(sec.glyphs[0].isSpace)
                            {
                                ...
                                {
                                    sec.glyphs[i].xAdvance += extraSpace * 1000 / this.size;
                                    currentDist += sec.glyphs[i].xAdvance * this.size / 1000 ;
                                }
                            }
                            else
                            {
                    ...
                    if(lineComplete)
                    {
                        tspanEl.endStringPos = endStringPos;
                        glyphCount += lineGlyphCount + (tspanEl.spaceRemoved ? 1 : 0);
                ...
                {
                    console.log('Found trailing spaces.');
                    ...
                    {
                        first = false;
                        console.log(spaceAdvance);
                        if(currentAdvance + (spaceAdvance - sliceAdvance) * this.size / 1000 > this.width)
                        {
                            let tmpAdvance = 0;
                            // Find the point to split
                            ...
                            let newSec: Section = {
                                ...
                                startGlyph: glyphCount + lineGlyphCount, stringStart: start + prevSlicePos
                            };
                           
                            tspanEl.sections.push(newSec);

                            lineGlyphCount += slicePos - prevSlicePos;
                            sliceAdvance += tmpAdvance;
                            endStringPos = start + slicePos - 1;

                            tspanEl.end += slicePos - prevSlicePos - 1;
                        ...
                        {
                            console.log('Should be adding space.');
                            ...
                            tspanEl.end += glyphs.length - prevSlicePos;
                            tspanEl.sections.push(newSec);
                            currentAdvance += (spaceAdvance - sliceAdvance);
                            endStringPos = lineEnd;
                            insertSpace = !insertSpace;
                            wordIdx++;
                            lineComplete = false;
                        }
                ...
                if(!lineComplete)
                {
                    ...
                    prevGlyphPos = tspanEl.start + lineGlyphCount;
                    glyphCount += lineGlyphCount;
        ...
        private processGlyphs(glyphs: Array<GlyphData>, style: TextStyle, word: string, stringStart: number)
        {
            ...
            {
                if(currentStyle.end <= tempGlyphs.glyphs[i].stringPositions[0] + stringStart)
                {
                ...
                let newGlyph: GlyphData = {
                    ...
                    colour: currentStyle.colour, uline: currentStyle.uline, oline: currentStyle.oline, tline: currentStyle.tline, xAdvance: tempGlyphs.positions[i].xAdvance,
        ...
        private processSpaceGlyphs(glyphs: Array<GlyphData>, word: string, stringStart: number)
        {
            ...
            for(let i = 0; i < word.length; i++)
            {
                if(word.charAt(i) == '\t')
                {
                    glyphs[i] =
                    {
                        xAdvance: SPACEDIST * TABWIDTH, xOffset: 0, yAdvance: 0, yOffset: 0, startAdvance: advance,
                ...
                else
                {
                    glyphs[i] =
                    {
                        xAdvance: SPACEDIST, xOffset: 0, yAdvance: 0, yOffset: 0, startAdvance: advance,
                        isLigature: false, isMark: false, codePoints: [9], path: '', stringPositions: [i],
                        colour: null, uline: null, oline: null, tline: null, isSpace: true, isHyphen: false
                    };
                }
        ...
        private stylesMatch(style1: Style, style2: Style)
        {
            return style1.colour == style2.colour && style1.oline == style2.oline && style1.uline == style2.uline && style1.tline == style2.tline
                && style1.weight == style2.weight && style1.style == style2.style;
        }
        ...
        private insertText(text: string, newStyle: Style)
        {
            ...
            console.log('Inserting text. String start is: ' + this.stringStart);
            ...
            for(let i = 0; i < this.styleSet.length; i++)
            {
                ...
                {
                    console.log("WENT INTO STRING START STYLE SPLIT." + i);
                    if(!hasInserted)
                    {
                ...
                {
                    if(sty.end < this.stringStart || (sty.end == this.stringStart && !this.stylesMatch(sty, newStyle)))
                    {
                        console.log("PUSHED STYLE BEFORE." + i)
            ...
            console.log('New style set is: ');
            console.log(styles);
            console.log('New style insert data is: ');
            console.log(newSty);
        ...
        private findStringPositions()
        {
            ...
            if(this.textNodes.length == 0 || this.cursorStart == 0)
            {
                this.stringStart = 0;
                return;
            }
            ...
            for(let i = 0; i < this.textNodes.length; i++)
            {
                ...
                if(line.end >= this.cursorStart)
                {
                    ...
                    for(let j = 0; j < line.sections.length; j++)
                    {
                        ...
                        if(this.cursorStart <= sec.startGlyph + sec.glyphs.length && this.cursorEnd >= sec.startGlyph)
                        {
                            // There is a special catch for the final glyph here, when the cursor is at the end of text we need the next glyph back.
                            for(let gPos = this.cursorStart - 1 > sec.startGlyph ? this.cursorStart - sec.startGlyph - 1 : 0; gPos < sec.glyphs.length; gPos++)
                            {
                                let glyph = sec.glyphs[gPos];

                                if(sec.startGlyph + gPos >= this.cursorStart && sec.startGlyph + gPos <= this.cursorEnd)
                                {
                                    for(let k = 0; k < glyph.stringPositions.length; k++)
                                    {
                                        if(newStringStart ==  null || glyph.stringPositions[k] + sec.stringStart < newStringStart)
                                        {
                                            newStringStart = glyph.stringPositions[k] + sec.stringStart;
                                        }

                                        if(sec.startGlyph + gPos < this.cursorEnd)
                                        {
                                            if(glyph.stringPositions[k] + sec.stringStart > largestStringPos)
                                            {
                                                largestStringPos = glyph.stringPositions[k] + sec.stringStart;
                                            }
    
                                            if(found[glyph.stringPositions[k] + sec.stringStart] === undefined && sec.startGlyph + gPos != this.cursorEnd)
                                            {
                                                found[glyph.stringPositions[k] + sec.stringStart] = true;
                                                this.selectedCharacters.push(glyph.stringPositions[k] + sec.stringStart);
                                            }
                                        }
                                    }
                                }
                                else if(sec.startGlyph + gPos == this.cursorStart - 1)
                                {
                                    for(let k = 0; k < glyph.stringPositions.length; k++)
                                    {
                                        if(glyph.stringPositions[k] + sec.stringStart > largestStringPos)
        ...
        {
            console.log('Word merger occuring: ' + undoStart + ', ' + undoEnd);
            console.log(this.cursorUndoPositions);
            console.log(this.cursorRedoPositions);
            ...
            if(this.prevWordStart != null && this.prevWordEnd != null)
            {
                ...
                this.wordEnd -= (undoEnd - undoStart) - 1;
                this.wordStart -= (undoEnd - undoStart) - 1;
                ...
                normReq.onsuccess = (event) =>
                {
                    if(normReq.result == null || normReq.result == undefined)
                    {
                        ...
                        normReqExt.onload = (oEvent) =>
                        {
                            ...
                            let buffer = NodeBuffer.Buffer(arrayBuffer, 0, arrayBuffer.byteLength);
...
// Google Code
fontList['NORMAL'] = { file: "https://storage.googleapis.com/wittylizard-168912.appspot.com/Fonts/NotoSans-Regular.ttf", ver: 1, style: "NORMAL" };
fontList['BOLD'] = { file: "https://storage.googleapis.com/wittylizard-168912.appspot.com/Fonts/NotoSans-Bold.ttf", ver: 1, style: "BOLD" };
fontList['ITALIC'] = { file: "https://storage.googleapis.com/wittylizard-168912.appspot.com/Fonts/NotoSans-Italic.ttf", ver: 1, style: "ITALIC" };
fontList['BOLDITALIC'] = { file: "https://storage.googleapis.com/wittylizard-168912.appspot.com/Fonts/NotoSans-BoldItalic.ttf", ver: 1, style: "BOLDITALIC" };
...
req.onupgradeneeded = (event: any) =>
{
    ...
    objectStore.transaction.oncomplete = (event) =>
    {
        ...
        {
            let arrayBuffer: ArrayBuffer = boldItalReq.response;
...
declare var NodeBuffer: { Buffer(buffer: ArrayBuffer, byteOffset: number, length: number): Buffer };
