...
abstract class BoardElement {
        ...
        callbacks: ElementCallbacks, serverId?: number, updateTime?: Date)
    {
        ...
        {
            this.idTimeout = setTimeout(this.idFailed, 10000, this);
        }
    ...
    private idFailed(self: BoardElement)
    {
        self.createAlert('CONNECTION ERROR', 'Unable to send data to server due to connection problems.');
...
//@ts-ignore
global.BoardElement = BoardElement;
...
//@ts-ignore
global.BoardPallete = BoardPallete;
...
//{
    ...
    class WhiteBoardWorker
    {
        ...
        endCallback()
        {
            ...
            let message: WorkerMessage =
            {
                ...
                infoMessages: controller.infoBuffer, removeAlertCount: controller.removeAlertCount, removeInfos: controller.removeInfoBuffer,
                selectCount: controller.currSelect.length, elementMoves: controller.elementMoves, elementDeletes: controller.elementDeletes,
                elementRestores: controller.elementRestores, clipboardData: clipData, viewUpdate: controller.viewUpdateBuffer,
                newViewCentre: controller.newViewCentre, newViewBox: controller.newViewBox
            }

            controller.removeAlertCount = 0;
        ...
        newAlert(type: string, message: string)
        {
            ...
            setTimeout(this.removeAlert.bind(this),2000);
        }

        removeAlert()
        {
            console.log("Added to remove alert.");
            this.removeAlertCount++;
            this.endCallback();
        }
        ...
        {
            self.addHoverInfo(id, mouseX, mouseY);
        }
            ...
            if(this.elementDict[data.serverId] == undefined || this.elementDict[data.serverId] == null)
            {
                if(this.components[data.type])
                {
                    ...
                    let callbacks: ElementCallbacks =
                    {
                        ...
                        // TODO: Element alerts
                        createAlert: (type: string, message: string) => { this.newAlert(type, message); this.endCallback(); },
    ...
    let registerComponent = (componentName: string, Element, Pallete) =>
    {
    ...
    //@ts-ignore
    global.registerComponent = registerComponent;
        ...
        onmessage = function(e)
        {
            ...
            switch(e.data.type)
            {
                case ControllerMessageTypes.START:
                    ...
                    {
                        console.log('Importing compoenent file: ' + e.data.componentFiles[i]);
