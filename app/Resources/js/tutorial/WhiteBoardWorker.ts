...
class WhiteBoardWorker
{
        ...
        if(e.buttons == 1)
        {
            if(mode == BoardModes.SELECT)
            {
                ...
                else if(this.currSelect.length == 1)
                {
                    ...
                    let retVal = elem.handleBoardMouseMove(e, changeX, changeY, this.components[elem.type].pallete);
    ...
    mouseUp(e: MouseEvent, mouseX: number, mouseY: number, downX: number, downY: number, pointList: Array<Point>, mode: string, scaleF: number, panX: number, panY: number)
    {
        let rectLeft;
        let rectTop;
        let rectWidth;
            ...
            if(this.currSelect.length == 0)
            {
                ...
                if(mode == BoardModes.SELECT)
                {
                    if(this.currSelect.length == 0)
                    {
                        ...
                        {
                            if(this.allowAllEdit || this.isHost || elem.user == this.userId)
                            {
    ...
    keyBoardInput(e: KeyboardEvent, inputChar: string, mode: string)
    {
        if(this.currSelect.length == 1)
        {
            ...
            let retVal = elem.handleKeyPress(e, 'Backspace', this.components[elem.type].pallete);
