...
class TutorialRoomController extends Controller
{
    ...
    {
        if($roomToken == "TestFun")
        {
            return $this->render('tutorial/room.html.twig', array('roomToken' => $roomToken, 'serverToken' => '', 'sessionId' => 0, 'isHost' => "FALSE"));
        }
        ...
        $roomPart = $em->getRepository('App:RoomParticipants')->findOneBy(array('roomId' => $room->getRoomId(), 'user' => $uid));
        ...
        $serverToken = 'https://127.0.0.1';
        if($kernel->getEnvironment() == 'prod')
        {
            $serverToken = 'https://www.wittylizard.com';
        }
        else if($kernel->getEnvironment() == 'test')
        {
            $serverToken = 'https://dev.wittylizard.com';
        }
    ...
    public function roomGetWaitAction($roomToken)
    {
        ...
        $tuteSess = $em->getRepository('App:TutorSession')->findOneBy(array('sessId' => $room->getRoomId()));
    ...
    public function roomTutorWaitAction($roomToken)
    {
        ...
        if(!$room->getServer())
        {
            $servers = $em->getRepository('App:TutorialServers')->findBy(array('isUp' => TRUE),array('numRooms' => 'ASC'));
    ...
    public function roomTutorAction($roomToken)
    {
        if($roomToken == "TestFun")
        {
            return $this->render('tutorial/room.html.twig', array('roomToken' => $roomToken, 'serverToken' => '', 'sessionId' => 0, 'isHost' => "TRUE"));
        }
